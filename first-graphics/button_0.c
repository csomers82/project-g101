...
void getNormCoords(int x, int y, float * nX, float * nY)
{
	Width = glutGet(GLUT_WINDOW_WIDTH);
	Height = glutGet(GLUT_WINDOW_HEIGHT);
	*nX = ((float)(x)) / ((float)(Width) / 2.0000f) - 1.0f;
	*nY = 1.0000f - ((float)(y)) / ((float)(Height) / 2.0000f);
}
...
*/
	...
	glutMouseFunc(Mouse);// mouse downclick function
